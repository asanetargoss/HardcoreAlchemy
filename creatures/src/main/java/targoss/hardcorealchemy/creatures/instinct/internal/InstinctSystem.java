...
public class InstinctSystem {
    ...
    public void tickPlayer(Configs configs, ICapabilityInstinct instinct, PlayerTickEvent event) {
        ...
        for (ICapabilityInstinct.InstinctEntry entry : instinct.getInstincts()) {
            ...
            for (InstinctNeedWrapper needWrapper : entry.getNeeds(player)) {
                ...
                if (!player.world.isRemote && instinctState.messenger.shouldSync()) {
                    HardcoreAlchemyCreatures.proxy.messenger.sendTo(new MessageInstinctNeedChanged(entryCount, needCountPerEntry, needWrapper), (EntityPlayerMP)player);
        ...
        if (needStatusChanged && !player.world.isRemote) {
            HardcoreAlchemyCreatures.proxy.messenger.sendTo(new MessageInstinctNeedState(instinct), (EntityPlayerMP)player);
        }
        ...
        if (!player.world.isRemote && effectChanges.size() != 0) {
            HardcoreAlchemyCreatures.proxy.messenger.sendTo(new MessageInstinctEffects(effectChanges), (EntityPlayerMP)player);
        }
